...
function [ V, D, it, flag ] = subspace_iter_v0( A, m, eps, maxit )
    ...
    B = randn(n,m); % générer une matrice aléatoire de taille n x m
    [Q,~] = qr(B,0); % décomposition QR sans calcul de R

    V = Q(:,1:m); % sélectionner les m premières colonnes de Q
